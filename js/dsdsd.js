...
function showDetailsOrder (event) {
  const categoryIndex = event.target.getAttribute('data-order');
  const order = arrLocalStorage[categoryIndex];
  console.log(order);
  const container = document.querySelector('.order_details');
  const elem = document.createElement('div');
  container.innerHTML = '';
  elem.innerHTML= `<p>Data of purchase: ${order.date} </p><p>${order.nameProdact} </p><p>${order.price}</p><p>${order.description} </p><p><b>Buyer information:</b></p><p>Full name: ${order.surname} ${order.firstName} ${order.patronymic}</p><p>City: ${order.city} </p><p>Branch № ${order.department}</p><p>Payment method: ${order.payment} </p><p>Quantity: ${order.amount} </p><p>Comment: ${order.comment} </p> `;
